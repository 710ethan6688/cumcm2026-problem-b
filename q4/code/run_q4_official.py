"""Run the Q4 strategy against the official HTTP simulator."""
...
def main() -> int:
    ...
    log_path = available_path(
        (args.log or results_dir / f"q4_official_{stamp}.jsonl").resolve()
    )
    output_path = available_path(
        (args.output or results_dir / f"q4_official_{stamp}.json").resolve()
    )
    ...
    summary = {
        ...
        **strategy.analytic_terminal_diagnostics,
        ...
        "average_localize_clear_time_s": (
            state.virtual_time_s / env.successful_clear_count
            if env.successful_clear_count
            else None
        ),
