"""Certified adaptive strip clearing terminal for Q4.

The planner rotates a conservative source polygon into the first-bearing
frame, partitions it into axial strips, and covers every strip intersection
by rectangles.  Each rectangle is assigned to its center and has half
diagonal no greater than the certified clearing radius.  The construction is
therefore a coverage certificate, not a sampled coverage claim.
"""
...
def estimate_full_clear_time_s(points: Sequence[Point], current: Point) -> float:
    """Worst-case route cost: all preceding clears fail and the last succeeds."""
    if not points:
        return 0.0
...
def build_certified_strip_plan(
    ...
    *,
    ...
    """Return the cheapest certified strip plan among several cell shapes."""
