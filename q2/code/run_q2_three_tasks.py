"""Q2 三项深度增强任务的可执行程序：
任务 1：近优候选区域栅格扫描、面积/包围盒/连通分量指标及等高线图；
任务 2：六个场景各运行 20 次的耗时基准与稳定性统计；
任务 3：物理安全缓冲量（b > 0）的敏感性分析。
"""
...
def run_task1_candidate_region_scan(
    ...
    # 计算等半径圆盘交集的精确轴对齐包围盒，并向内对齐到栅格，
    # 避免人为选取的绘图区间悄然截断所声称的全域扫描范围。
    ...
    plot_path = None
    if save_plot:
        # 结果由两个分离的叶瓣构成；使用两个局部放大面板可让论文尺寸下的
        # 几何结构清晰可读，并避免中央出现大块空白。
...
def run_task3_safety_buffer_sensitivity() -> dict[str, object]:
    ...
    print(">>> Running Task 3: Safety buffer (b > 0) sensitivity analysis...")
