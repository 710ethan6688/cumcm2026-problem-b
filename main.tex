...
针对\textbf{问题四}全向与半平面定向混合源非对称遮蔽挑战，提出\textbf{25点双环三角网骨架}与\textbf{双轨驱动清除策略}。证明了双环三角网测站（最大间距 $\mathbf{983.60\text{ m}}$，全域内切圆半径 $\mathbf{1806.28\text{ m}}$）对任意未知朝向目标提供视场覆盖保证；构建由全局基准搜索组成的理论保证主链，结合自适应尾部前瞻探测构成的轻量启发支路，抑制盲侧无效机动与网格长尾。100 组配对对照实验平均缩减任务时间 $\mathbf{109.8\text{ s}}$（难例降低 $\mathbf{646.5\text{ s}}$），无效尝试减少 $\mathbf{40.2\%}$；官方接口 17 轮演练保持 $\mathbf{100\%}$ 清除率，单源平均耗时 $\mathbf{530.41\text{ s}}$。
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
    \item \textbf{目标位置静止与基准坐标无漂移}：目标干扰源在整个作业过程中保持静止；机器狗的高精度差分惯导系统运行良好，其自身坐标与绝对方位基准无测量误差。
    \item \textbf{观测数据同源匹配与有效感知}：在同一频段内提取的连续测向数据已完成前置同源聚类，且参与解算的各次测向均处于目标有效覆盖半径内，为有效观测。
    \item \textbf{确定性区间有界误差}：测向仪角度误差服从未知但有界的确定性区间分布（误差半宽 $\varepsilon = 1^\circ$），不作先验高斯分布假设，遵循最劣工况保障原则。
    \item \textbf{同点重复观测误差恒定}：在同一物理坐标点不移动时的重复测向受相同的局部近场多径与地形畸变影响，误差值保持恒定，无法通过原地多次采样平均来消除测向偏差。
    \item \textbf{分段匀速直线机动与加减速忽略}：机器狗转向、起步与制动过程极为迅速，与数百米至千米级的长距离巡航耗时相比属于微小量，可近似为以 $5\text{ m/s}$ 速度运行的二维质点分段匀速直线运动。
    \item \textbf{二维视距传播与辐射源状态稳定}：忽略地表微小起伏，电磁信号在二维平面内沿视距直线传播；目标干扰源在巡航作业期内发射功率平稳且工作频点固定，不存在动态跳频或随机启闭现象。
\end{enumerate}
