...
本文研究四足机器狗对突发未知无线电干扰源的自主搜索定位与物理清除策略。面对干扰源参数未知、测向误差有界以及多目标在线搜索等复杂现实约束，本文结合机器狗运动学特征与传感器角域属性，建立了一套分层序贯决策与路径规划模型。通过引入计算几何与启发式在线调度算法，依次攻克了多站交会定位、第二检测点选址、全向干扰源巡航清除及混合干扰源双轨协同等核心问题。

针对问题一，构建了测向误差确界约束下的交会定位与多边形几何直径解算模型。考虑到 $\pm 1^\circ$ 的测向误差，首先将各站实测示向度转化为半平面不等式系统，并采用极角排序半平面交算法求得定位多边形的极点序列。利用凸分析极值原理，证明区域直径必在极点对处取得，进而借助旋转卡壳算法快速提取最远对踵点。基准算例表明定位多边形几何直径为 $\mathbf{48.523\text{ m}}$，且该算例下闭圆盘实现 100\% 完全覆盖。随后构造正三角形几何反例，证明在一般情形下以该最远对踵点连线为直径的闭圆盘无法必然覆盖整个定位区域（1000 组蒙特卡洛测试的完全覆盖率为 $\mathbf{34.7\%}$）。经全顶点穷举验证，算法一致率达 $\mathbf{100\%}$，且 8443 次单调性测试均顺利通过。

针对问题二，以第二检测点的二维空间坐标为决策变量，构建了旨在最小化“后验最劣几何直径”的极小化极大 (Minimax) 优化模型。为缩小求解空间，针对由初测示向角与感知极限围成的凸多边形包络，利用角平分线正交变换消除了空间对称冗余。在满足机动安全距离的前提下展开离散网格搜索，锁定了最优第二检测点坐标 (839.64, 538.75)，机动距离为 $\mathbf{997.62\text{ m}}$。执行二次交会后，最劣后验直径由单站的 $1500.23\text{ m}$ 大幅缩减至 $\mathbf{111.90\text{ m}}$，不确定空间压缩比高达 $\mathbf{92.5\%}$。扩展实验进一步揭示了该策略有效拉大交会视角、抑制径向拉伸的几何机理。

针对问题三，面向全域多全向干扰源的协同清除任务，设计了融合拓扑覆盖保证与滚动调度的自主巡航模型。借助平面覆盖理论，严格证明了间距为 $\mathbf{1732.05\text{ m}}$ 的正六边形蜂窝测站能够对半径 $\mathbf{1800\text{ m}}$ 的目标圆域实现无盲区覆盖，并确保交会多边形始终包含目标真值。在此基础上，确立了以 $\mathbf{19.5\text{ m}}$ 最小外接圆半径为阈值的安全清除判据，配套设计了网格遍历兜底与频道单向吸收退出准则，从理论上保证系统必能在有限步内以 100\% 清除率终止。官方模拟器的三次正式测试显示，全向源清除率均达 $\mathbf{100\%}$，平均耗时为 $\mathbf{3162.4\text{ s}}$（单源加权耗时 $\mathbf{243.3\text{ s}}$），单步决策时间不足 $\mathbf{0.001\text{ s}}$。

针对问题四，立足于全向与半平面定向混合干扰源的非对称遮蔽对抗，建立了一套基于多视角包围拓扑与双轨协同决策的闭环清除模型。通过分析凸包分离性质，严格证明目标落入测站闭凸包是实现全向可观测的充要条件。进而，采用最大边长 $\mathbf{983.60\text{ m}}$、内切圆半径 $\mathbf{1806.28\text{ m}}$ 的 25 点双环三角网测站拓扑，保障了对全域未知朝向目标的可靠发现。在主干算法具备坚实理论支撑的基础上，引入了自适应的尾部前瞻探测轻量化支路。100 组配对对照实验证实，该启发支路使平均任务耗时缩减了 $\mathbf{109.8\text{ s}}$（困难样本缩减 $\mathbf{646.5\text{ s}}$），无效尝试降低 $\mathbf{40.2\%}$。官方模拟器测试中混合源清除率维持 $\mathbf{100\%}$，任务平均耗时为 $\mathbf{6759.9\text{ s}}$（单源加权耗时 $\mathbf{533.7\text{ s}}$）。

总体而言，本文方案均通过了独立多算法交叉复核与极端工况压力测试，不仅严格契合平台机动性能与环境物理约束，更兼具完备的理论保障与毫秒级的在线响应能力。
...
四个核心问题层层递进、环环相扣：\textbf{问题一为基石}（提供底层半平面交会与旋转卡壳几何计算引擎）；\textbf{问题二为纽带}（攻克 Minimax 主动选点与不确定性压缩技术）；\textbf{问题三为骨架}（建立蜂窝无盲区覆盖、MEB 精准清除与多频段在线调度框架）；\textbf{问题四为升华}（建立多视角凸包覆盖定理与双轨协同策略，攻克非对称定向遮蔽对抗难题）。
...
为兼顾全局搜索视野与计算效率，采用全域粗筛、局部寻优与精细评估三阶段求解算法，整体决策求解流程如图 \ref{fig:q2_flowchart} 所示：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.2em]
...
\noindent\textbf{推论}：任意客观存在的干扰源在遍历 $\mathcal{Q}_7$ 过程中必被至少一个测站探测捕获；若某一频道在 $\mathcal{Q}_7$ 全部 7 站均无信号，可严格确证该频道无目标，标记为空置 $\mathcal{S}_{\mathrm{abs}}$。七点蜂窝全域完备覆盖构型及机器狗执行巡航定位与激光清除的实测机动轨迹如图 \ref{fig:q3_seven_point_coverage} 所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\linewidth]{images/fig_q3_seven_point_coverage.png}
    \caption{七点正六边形对称蜂窝全域覆盖几何构型(a)与机器狗巡航清除实测轨迹(b)}
...
\begin{table}[htbp]
    ...
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}l c c c c c c c}
        ...
        \midrule
        正式测试 1 & 11 个 & 11 个 & 100\% & 33 站 & 103 次 & $2741.0\text{ s}$ & $3.18\text{ s}$ \\
        正式测试 2 & 14 个 & 14 个 & 100\% & 46 站 & 90 次 & $3378.6\text{ s}$ & $4.60\text{ s}$ \\
        正式测试 3 & 14 个 & 14 个 & 100\% & 44 站 & 98 次 & $3367.7\text{ s}$ & $4.86\text{ s}$ \\
        \midrule
        \textbf{加权均值} & 13.0 个 & 13.0 个 & \textbf{100\%} & 41.0 站 & 97.0 次 & \textbf{3162.4 s} & \textbf{4.22 s} \\
        \bottomrule[1.5pt]
    \end{tabular*}
\end{table}
...
测试结果与理论机制分析如下：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.2em]
    \item \textbf{完全清除与精准打击}：三次测试清除率均达到 100\%，全部 39 个目标均在 MEB 判据下一次清除成功，无任何误判或无效清除；单源平均定位清除时间分别为 $249.18\text{ s}$、$241.33\text{ s}$ 与 $240.55\text{ s}$（加权均值 $243.26\text{ s/源}$）；
    ...
    \item \textbf{时间合规与实时高效}：全任务虚拟时间最长为 $3378.6\text{ s}$（约 56.3 分钟），完全处于 100 分钟限制之内；算法在线决策计算总耗时均值仅 $4.22\text{ s}$（单步决策低于 $0.1\text{ ms}$），完全满足在线实时决策要求。
\end{enumerate}
