...
\begin{cumcmabstract}
\vspace{-0.2em}
针对突发性未知无线电干扰源的机动侦测与物理清除任务，本文结合计算几何理论、极值优化方法与在线序贯决策，构建了一套兼具理论保证与实时计算效率的机动搜索、定位与清除策略体系。

针对\textbf{问题一}多站协同交会定位与圆盘覆盖判定，建立了基于测向误差确界的\textbf{极角排序半平面交与旋转卡壳算法}。将测向角域严格转化为半平面不等式组求解凸多边形极点序列；证明区域直径必在极点处取得，利用旋转卡壳法提取最远对踵点，求得基准算例下定位多边形几何直径为 $\mathbf{48.523\text{ m}}$。以最远点对连线为直径构造闭圆盘，由钝角三角形反例与点积判据严格证明了圆盘并非恒能覆盖定位区域；在 1000 组蒙特卡洛算例中，完全覆盖率为 $\mathbf{34.7\%}$。全顶点穷举对照表明算法一致率达 $\mathbf{100\%}$，8443 次单调性测试全部通过。

针对\textbf{问题二}第二检测点优选，建立了以\textbf{最劣后验几何直径极小化}为目标的序贯优化模型。以初测示向角与物理感知确界划定外包络凸集并引入机动安全边界；通过角平分线正交基底消除对称关系，采用离散最坏场景采样全局寻优。基准算例下求得推荐第二检测点坐标为 $\mathbf{(839.64, 538.75)\text{ m}}$，机动距离为 $\mathbf{997.62\text{ m}}$，最劣后验直径由 $1500.23\text{ m}$ 降至 $\mathbf{111.90\text{ m}}$，不确定空间压缩比达 $\mathbf{92.5\%}$。扩展测试验证了拉大交会视角、抑制径向拉伸的几何机理。

针对\textbf{问题三}全向干扰源全域自主搜索清除任务，提出基于\textbf{七点蜂窝拓扑骨架}与\textbf{在线协同贪心决策}的主算法。证明了间距 $1732.05\text{ m}$ 的正六边形蜂窝测站对半径 $1800\text{ m}$ 目标圆形区域的无盲区覆盖，以及交会凸多边形恒包含目标真实位置；确立了以最小外接圆半径 $\mathbf{19.5\text{ m}}$ 为阈值的精确定位清除判据，设计了有限网格兜底策略与全频道单向吸收退出准则，证明了系统在有限步内必以 100\% 清除率终止。规范模拟器三次测试清除率均达 $\mathbf{100\%}$，平均任务时间 $\mathbf{5127.3\text{ s}}$，单步决策低于 $\mathbf{0.001\text{ s}}$。

针对\textbf{问题四}全向与半平面定向混合源非对称遮蔽挑战，提出\textbf{25点双环三角网骨架}与\textbf{双轨驱动清除策略}。证明了双环三角网测站（最大间距 $\mathbf{983.60\text{ m}}$，全域内切圆半径 $\mathbf{1806.28\text{ m}}$）对任意未知朝向目标提供视场覆盖保证；构建由全局基准搜索组成的理论保证主链，结合自适应尾部前瞻探测构成的轻量启发支路，抑制盲侧无效机动与网格长尾。100 组配对消融实验平均缩减任务时间 $\mathbf{109.8\text{ s}}$（难例降低 $\mathbf{646.5\text{ s}}$），无效尝试减少 $\mathbf{40.2\%}$；官方接口 17 轮演练保持 $\mathbf{100\%}$ 清除率，单源平均耗时 $\mathbf{530.41\text{ s}}$。
...
命题 1 将连续搜索降维至 $\binom{h}{2}$ 对离散顶点比较，但全顶点穷举计算复杂度仍为 $O(h^2)$。根据对踵几何特征，两点最大间距必在相互平行的外切支撑线接触点（对踵点对）处取得。为此引入旋转卡壳算法，通过一对平行卡尺绕多边形旋转步进，仅需在 $O(h)$ 线性时间内遍历全部对踵候选，即可精确提取区域几何直径 $D$ 及最远顶点对 $(\mathbf{a}^*, \mathbf{b}^*)$。
...
\end{equation}
其中 $\eta_{\mathrm{clear}} = K/N = 1$。为此，本文兼顾数学理论推导与在线调度时效：一方面建立涵盖全域几何覆盖、真值区域包含、安全清除与自主退出的理论保证，在数学上杜绝目标遗漏；另一方面采用极小极大几何收缩准则与动态优先级评估，实现多频道目标在线交错搜索、追踪与就地清除，大幅压缩行进与测向时间。
...
\noindent\textbf{计算复杂度与执行时间}：系统各频道凸外包多边形极点数严格满足 $h \le 8 + 3 \times 2 = 14$，空间存储量为 $O(|\mathcal{C}| \cdot h) = O(1)$ 常数级；单步半平面求交与 Welzl 线性最小外接圆算法耗时不超过 $0.1\text{ ms}$，单步总决策耗时稳定在 $2\sim 5\text{ ms}$；全任务实际计算总耗时低于 $3\text{ s}$，算法运行极其高效，完全满足机器狗在线自主决策的实时性要求。
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.06em, topsep=0.05em]
    \item \textbf{背向静默与无信号的多重成因分析}：定向干扰源仅在其未知辐射朝向确定的 $180^\circ$ 闭半平面内发射电磁信号。当机器狗停靠检测未收到信号时，该观测反馈不再等价于空间距离超出物理覆盖上限，而可能由频道空置、目标已清除、空间距离超距、或测站落入目标辐射盲侧四类互斥物理状态析取而成。因此，不能依据单次检测无信号直接排除几何空间。
    \item \textbf{单一测站覆盖准则失效与多视角包围要求}：在全向辐射条件下，任意测站与其周围 $1000\text{ m}$ 圆盘内的目标均可建立双向通信；而在定向辐射条件下，即使测站与目标距离小于 $1000\text{ m}$，若测站位于目标背向则依然无法探测。要对任意未知朝向的目标实现无遗漏发现，目标必须被周围多个邻近测站几何包围，从而使得无论目标朝向何方，至少存在一个测站落入其有效发射半平面内。
    \item \textbf{有限探测成本与长尾清除耗时的权衡}：在获得初始示向后，若直接机动至候选测站可能再次陷入目标发射盲侧而失联；若完全依赖网格搜索，则会导致任务时间随网格单元遍历而急剧膨胀。因此，必须在理论保证主链之外，建立能够识别盲侧并自适应终止追加测向的轻量决策支路。
\end{enumerate}

为此，本文确立保证链与效率支路协同求解思路：一方面构建具备多视角局部凸包包围特性的基准搜索骨架，在数学上证明对任意未知朝向目标的完备发现能力；另一方面构建集距离安全选点、对称镜像重捕获与自适应尾部前瞻于一体的在线决策体系，在确保全部清除的前提下降低任务耗时。
...
\end{equation}
其中 $e_c \in \{0, 1\}$ 表示频道 $c$ 是否客观存在物理目标（总目标数满足 $N = \sum_{c=1}^{20} e_c \in [10, 16]$；若 $e_c = 0$ 则后序分量无物理定义）；目标二维空间坐标记为 $\mathbf{x}_c = (x_{c,1}, x_{c,2})^\top \in \Omega$；$R_{c} \in [1000, 1500]\text{ m}$ 为该目标的实际最大电磁覆盖半径；$\tau_c \in \{\mathrm{O}, \mathrm{D}\}$ 表征天线辐射类型（$\mathrm{O}$ 为全向辐射，$\mathrm{D}$ 为半平面定向辐射）；$\mathbf{u}_c = (\cos\phi_c, \sin\phi_c)^\top$ 为定向目标的单位法向辐射方向向量（若 $\tau_c = \mathrm{O}$ 则该项不具方向约束）。
...
为消除歧义，将无信号观测事件分解为四类两两不相交的物理事件之并：$E_{\mathrm{none}} = E_0 \cup E_1 \cup E_2 \cup E_3$（$E_i \cap E_j = \emptyset, \forall i \neq j$）。其中：$E_0 = \{e_c = 0\}$（频道未布设目标）；$E_1 = \{e_c = 1, h_c^k = 1\}$（目标此前已被清除）；$E_2 = \{a_c^k = 1, \|\mathbf{q} - \mathbf{x}_c\| > R_c\}$（目标存活但超出接收半径）；$E_3 = \{a_c^k = 1, \|\mathbf{q} - \mathbf{x}_c\| \le R_c, \tau_c = \mathrm{D}, \mathbf{u}_c^\top (\mathbf{q} - \mathbf{x}_c) < 0\}$（目标存活且在视距内但处于发射盲侧）。
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
当返回有效示向角 $\hat{\theta}_c$ 时，真实方位角 $\theta_c^\ast = \operatorname{atan2}(x_{c,2} - q_2, x_{c,1} - q_1)$ 与实测示向角满足圆周角距离有界约束：$d_{\mathbb{S}^1}(\theta_c^\ast, \hat{\theta}_c) = |\operatorname{wrap}_{[-180^\circ, 180^\circ)}(\theta_c^\ast - \hat{\theta}_c)| \le \varepsilon = 1^\circ$。算法在数值交会中采用 $\varepsilon_{\mathrm{tol}} = 1.005^\circ$ 的微小数值容差，避免浮点截断真实解集。作业全流程中各测量分支均实行统一的近距终止准则：若任意测站检测返回 $\texttt{near}$（确证目标处于 $5\text{ m} \le 20\text{ m}$ 清除覆盖区），机器狗即刻在当前停靠位置直接实施激光清除，并置该频道状态为已清除，闭合所有在线反馈状态分支。

\subsubsection{频道追踪信息状态与物理时间模型}

机器狗针对每个频道 $c \in \mathcal{C}$ 独立维护五元追踪信息状态 $\mathcal{I}_c^k = (s_c^k, P_c^k, \mathcal{M}_c^k, \mathcal{V}_c^{\mathrm{hist}}, \mathcal{B}_c^{\mathrm{hist}})$：包含离散状态标示 $s_c^k \in \{\text{未探明}, \text{已发现}, \text{定位中}, \text{网格兜底}, \text{已清除}, \text{确证空置}\}$；紧致凸多边形位置外包 $P_c^k \subset \mathbb{R}^2$（初始为目标圆外切正四十八边形）；25 点骨架完成集 $\mathcal{M}_c^k \subseteq \{1, \dots, 25\}$；以及历史正观测站集 $\mathcal{V}_c^{\mathrm{hist}}$ 与盲侧测站集 $\mathcal{B}_c^{\mathrm{hist}}$。
...
\noindent\textbf{推论 1（最小接收半径下的稳健充分保证）}\quad 针对接收半径确界 $R_c \in [1000, 1500]\text{ m}$ 工况，若在保底半径 $R_{\min} = 1000\text{ m}$ 下满足 $\mathbf{x} \in \operatorname{conv}(\mathcal{S}_{1000}(\mathbf{x}))$，则是目标在全工况下全方向可被发现的统一充分条件。

基于命题 5 与推论 1，在目标区域 $\Omega$ 内构造由 25 个特征测站组成的双环三角剖分拓扑构型：中心测站位于原点 $(0, 0)$；内环 12 点分布于半径 $R_1 = 950\text{ m}$ 圆周，极角为 $i \times 30^\circ$；外环 12 点分布于半径 $R_2 = 1870\text{ m}$ 圆周，极角交错旋转 $15^\circ$（$j \times 30^\circ + 15^\circ$）。中心点与内环构成 12 个三角形，内外环交错构成 24 个三角形，共 36 个三角形单元。几何构型如图 \ref{fig:q4_25point_coverage} 所示，参数验证汇总于表 \ref{tab:q4_topology_params}。

\begin{figure}[htbp]
    \centering
    \vspace{-0.3em}
    \includegraphics[width=0.22\linewidth]{images/fig_q4_25point_coverage.png}
    \vspace{-0.4em}
...
\noindent\textbf{引理 1（确定性测向误差下交会凸多边形维持目标真值包含）}\quad 设初始几何外包满足真实位置 $\mathbf{x}_c \in P_c^1$。在后续各测站 $\mathbf{q}_k$ 处获取的有效测向角域 $\mathcal{W}_k$ 严格覆盖真实物理方位角，且仅使用有效正示向通过半平面求交更新，则生成的凸多边形序列恒满足真值包含关系：$P_c^{k+1} = P_c^k \cap \mathcal{W}_k \implies \mathbf{x}_c \in P_c^{k+1}, \forall k \ge 1$。
...
\subsubsection{双阈值清除与有限网格兜底判据}

\noindent\textbf{判据 2（双阈值直接清除与 26 米正方形网格遍历兜底）}\quad 针对频道 $c$ 凸多边形可行域 $P_c^k$，采用 Welzl 算法计算其最小外接圆半径 $\rho(P_c^k)$ 及圆心 $\mathbf{c}_c$：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.04em, topsep=0.04em]
    \item \textbf{常规定位提前清除}：若 $\rho(P_c^k) \le 19.5\text{ m}$，机动至 $\mathbf{c}_c$ 实施激光清除。其中 $20.0\text{ m}$ 为理想执行条件下的理论充分清除半径，采用 $19.5\text{ m}$ 预留 $0.5\text{ m}$ 的数值计算与机动执行工程裕量；
    \item \textbf{尾部探测后清除}：在完成最后一次尾部追加探测后，若后验外接圆半径满足理想执行充分清除界 $\rho(P_c^{k+1}) \le 20.0\text{ m}$，直接机动至新圆心实施清除；
    \item \textbf{有限网格遍历兜底}：若未达上述条件，对 $P_c^k$ 的外接水平矩形作边长 $\Delta g = 26\text{ m}$ 正方形网格划分，完整保留所有与 $P_c^k$ 存在几何相交的网格单元，沿蛇形次序依次遍历各相交单元中心实施清除。
\end{enumerate}

\noindent\textbf{引理 2（有限网格遍历清除保证）}\quad 边长 $\Delta g = 26\text{ m}$ 正方形网格中心至单元内任意点的最大距离为半对角线长 $r_{\mathrm{cell}} = \frac{\sqrt{2}}{2} \times 26 = 18.385\text{ m} < 20.0\text{ m}$。只要真实目标包含于相交单元并集内，有限步遍历各单元中心点必能成功实现清除。

\subsubsection{混合源全场无残留自主退出准则}

\noindent\textbf{准则 3（混合干扰源全域完全清除充分退出证书）}\quad 系统仅在以下任一充分证书成立时，签发任务完成证书并终止主循环：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.04em, topsep=0.04em]
    \item \textbf{目标全数证书}：累计成功清除的独立目标频道数达到理论上限 $|\mathcal{C}^{\mathrm{clear}}| = 16$；
    \item \textbf{频道完备结案证书}：所有已捕获目标均已成功清除，且其余频道在 25 点基准测站中均完成实测且全部明确返回 $\texttt{no\_signal}$，即满足 $\forall c \in \mathcal{C}, c \in \mathcal{C}^{\mathrm{clear}} \lor [ \mathcal{M}_c = \{1, \dots, 25\} \land \forall i \in \{1, \dots, 25\}, y_{c,i} = \texttt{no\_signal} ]$。通信超时或响应未知不得计入 $\texttt{no\_signal}$。
\end{enumerate}
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.04em]
    \item \textbf{条件对称镜像重捕获}：若主动测向预算尚未耗尽，算法沿首次示向中轴线将失联测站对称反射至对侧几何位置 $\mathbf{q}_{\mathrm{mirror}} = \mathbf{q}_1 + 2 [ (\mathbf{q}_2 - \mathbf{q}_1)^\top \mathbf{u}(\hat{\theta}_1) ] \mathbf{u}(\hat{\theta}_1) - (\mathbf{q}_2 - \mathbf{q}_1)$，在重新校验顶点距离安全后前往探测，提高落入有效发射半平面的概率。
    \item \textbf{自适应尾部前瞻探测}：在可行域内离散生成 96 个位置与 36 个法向朝向样本构成的集合，构建相容于历史观测的样本子集 $\mathcal{H}_k$。若 $\mathcal{H}_k = \emptyset$ 则不触发前瞻；否则定义候选测站的等权几何相容状态代理量：
    \begin{equation}
        p_v(\mathbf{q}) = \frac{\#\{(\mathbf{x}_i, \mathbf{u}_j) \in \mathcal{H}_k \mid \mathbf{q} \text{ 对该状态满足可接收条件}\}}{\#\mathcal{H}_k} \in [0, 1]
    \end{equation}
\end{enumerate}

\noindent\textbf{判据 3（基于几何相容代理与单步前瞻成本权衡的尾部探测）}\quad 设当前相交网格遍历耗时预估为 $T_{\mathrm{grid}}$（包含网格进入位移、试探与最终清除时间），候选探测耗时为 $T_{\mathrm{probe}} = d/v + 5\text{ s} + \mathbb{I}_{\mathrm{switch}} \times 1\text{ s}$（$\mathbb{I}_{\mathrm{switch}}$ 为切频指示），非空相交示向集为 $\Theta_k^+(\mathbf{q}) = \{\theta \in \Theta_{\mathrm{sample}} \mid P_c^k \cap \mathcal{W}(\mathbf{q}, \theta, \varepsilon_{\mathrm{tol}}) \neq \emptyset\}$，样本化后验最劣直径代理为 $D_{\mathrm{post}} = \max_{\theta \in \Theta_k^+(\mathbf{q})} \operatorname{diam}(P_c^k \cap \mathcal{W}(\mathbf{q}, \theta, \varepsilon_{\mathrm{tol}}))$（若 $\Theta_k^+(\mathbf{q}) = \emptyset$ 则不触发前瞻），当前直径为 $D$。当几何相容代理量满足固定门槛 $p_v \ge 0.30$ 且代理收益高于探测代价时，追加一次终端探测：
\begin{equation}
    T_{\mathrm{probe}} < T_{\mathrm{grid}} \cdot p_v \left[ 1 - \left( \frac{D_{\mathrm{post}}}{D} \right)^2 \right], \quad p_v \ge 0.30
\end{equation}
若探测返回 $\texttt{near}$ 则在当前测站实施激光清除；若探测获取有效示向，则完成位置外包最后一次裁剪，并在其最小外接圆半径满足 $\rho(P_c^{k+1}) \le 20.0\text{ m}$ 时机动至新圆心直接清除，否则转入 26 米相交网格遍历清除；若再次返回 $\texttt{no\_signal}$ 或判据不成立，则终止追加测向，立即转入相交网格遍历清除。
...
\begin{figure}[htbp]
    \centering
    \vspace{-0.3em}
    \includegraphics[width=0.26\linewidth]{images/fig_q4_system_architecture.png}
    \vspace{-0.4em}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\linewidth}{l X}
        ...
        步骤 2 & 检查准则 3 退出证书：若满足则结束任务并输出报告，否则进入调度循环； \\
        步骤 3 & 优先检查是否存在满足直接清除条件频道（$\rho \le 19.5\text{ m}$）：若存在则机动至圆心实施清除； \\
        步骤 4 & 检查定位中频道（主动测向 $< 2$ 次）：选取顶点距离安全（$d_{\max} \le 995\text{ m}$）测站机动测向；若反馈 $\texttt{near}$ 则在当前位置直接清除；若获示向则求交裁剪；若无信号则评估对称镜像点； \\
...
\noindent\textbf{命题 6（理想同步执行条件下混合干扰源搜索清除有限步终止）}\quad 在目标空间静止、单源单频、总目标数不超过 16、最小接收半径不小于 1000 米、确定性测向误差确界为 1 度、通信响应确定、调度公平无饥饿、及无外部提前终止的理想同步条件下，算法 2 必在有限离散动作步数内完成全部实际存在目标的清除并安全退出。

\begin{cumcmproof}
在搜索清除任务全过程中，系统可生成的离散决策动作总量存在有限确定上界：全向—定向 25 点基准搜索任务至多 $20 \times 25 = 500$ 次；实际存在目标至多 16 个，每个目标常规主动测向至多 2 次、尾部探测至多 1 次；根据引理 1，凸多边形始终维持真值包含；对于进入网格阶段的目标，由于其位置外包紧致有界，其 26 米相交网格单元数必然有限，且每个单元至多访问一次。在无饥饿公平调度策略下，系统执行动作持续单调消耗未完成任务集合，绝不陷入死循环，算法必在有限离散动作步数内完成全部实际存在目标的清除并触发准则 3 退出证书。
\end{cumcmproof}
...
为检验自适应尾部前瞻探测对缩减任务耗时的具体作用，在 100 组随机生成的同分布场景（随机种子 0 至 99）下开展同场景配对消融实验，对比未启用尾部探测的方案（V2）与本文完整方案（V3）。统计数据汇总于表 \ref{tab:q4_ablation_results}。
...
\begin{enumerate}[leftmargin=2.2em, itemsep=0.01em, topsep=0.01em]
    ...
    \item \textbf{保证主链与效率支路协同分工}：以基准多视角覆盖和有限网格遍历承担完备清除保证，以距离安全探测和尾部前瞻优化动作次序；
