...
\subsection{问题特性与整体策略框架}

\subsubsection{问题难点与建模考量}

问题三要求机器狗在半径 $R_{\mathrm{target}} = 1800\text{ m}$ 的圆形区域内，自主搜索、定位并清除全部未知的全向干扰源，在确保无一遗漏的前提下极小化任务总时间。该任务的核心难点在于：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
    \item \textbf{目标先验信息未知}：目标数量 $N \in [10, 16]$、所在频段 $\mathcal{C} = \{1, \dots, 20\}$、位置坐标 $\mathbf{s}_c$ 及有效接收半径 $R_c \in [1000, 1500]\text{ m}$ 事先均未知；
    \item \textbf{空间观测存在区域差异}：测向传感器存在确界误差 $[-1^\circ, 1^\circ]$；当间距 $\le 5\text{ m}$ 时信号过强无法测向，但可直接实施清除；$5\sim 20\text{ m}$ 内兼具测向与清除条件；$20\text{ m}\sim R_c$ 仅可测向；超出 $R_c$ 则无信号响应；
    \item \textbf{缺乏全局完成提示}：外部环境不主动提供剩余目标数，需算法根据搜索与清除记录，自主确证全场目标已无遗漏并主动决断终止。
\end{enumerate}

\subsubsection{优化目标与策略设计}
...
\end{equation}
其中 $\eta_{\mathrm{clear}} = K/N = 1$。为此，整体策略包含两个核心层次：
\begin{itemize}[leftmargin=2.5em, itemsep=0.15em]
    \item \textbf{理论完备性保障}：通过几何覆盖、相容集真值包含与外接圆判据，提供全域搜索无盲区、清除高精度与全局终止无遗漏的数学证明；
    \item \textbf{在线动态优化}：在满足上述完备性约束的前提下，采用极小极大几何收缩准则与动态优先级评估，实时规划机器狗的移动路径与测向顺序。
\end{itemize}
...
\subsection{全域无遗漏清除的理论证明链}

为保证 100\% 清除率并自主决断终止，建立四环递进的逻辑证明链：
\[
\begin{aligned}
    \text{全域蜂窝覆盖保证发现} &\;\Longrightarrow\; \text{误差归纳恒包真值} \\
    &\;\Longrightarrow\; \text{最小外接圆精准清除} \;\Longrightarrow\; \text{完备终止充分判据}
\end{aligned}
\]

\subsubsection{七点蜂窝覆盖与保证发现定理}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{images/fig_q3_seven_point_coverage.png}
    \caption{七点蜂窝全域完备覆盖几何验证}
    \label{fig:q3_seven_point_coverage}
\end{figure}

\subsubsection{有界误差包含性与清除判定定理}
...
定义多边形 $K_{c, k}^+$ 的最小外接圆（MEB）半径为 $r(K_{c, k}^+)$，圆心为 $\mathbf{c}(K_{c, k}^+)$。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\linewidth]{images/fig_q3_geom_meb_grid.png}
    \caption{角楔交会、MEB判据与网格兜底机制示意图}
    \label{fig:q3_geom_meb_grid}
\end{figure}

\subsubsection{全局任务完备终止充分性定理}

记已清除频道集为 $\mathcal{C}_k^{\mathrm{cl}}$，已证实空置频道集为 $\mathcal{C}_k^{\mathrm{abs}}$。
...
实测数据表明：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
    \item 在各类极限工况场景中，系统目标清除率均稳定达到 100\%；在 15\% 通信丢包或超时干扰下，重试机制成功处理了全部 394 次未决请求；在强制网格兜底工况下，虽因全面遍历搜索使耗时上升至 $74833.4\text{ s}$，但依然实现了全部目标的有效清除，印证了定理 4 与定理 6 的理论正确性。
    \item 策略消融表明，极小极大主动选点使平均任务时间由 $72982.0\text{ s}$ 大幅缩减 93.4\% 至 $4800.5\text{ s}$；边搜边清的协同机制避免了多余的往返移动，使任务耗时进一步降低 20.5\%；而缺乏几何覆盖保障的对照基准 C 出现了明显的目标漏检，单目标清除率仅 90.6\%（全场完全清除率仅 32.0\%），无法满足全部清除的硬性要求。
\end{enumerate}
...
测试结果与理论机制分析如下：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
    \item \textbf{完备性与精准性}：三次测试清除率均严格达到 100\%，且全部目标均在 MEB 判据下一次清除成功，清除动作次数与真实目标数完全一致，无任何误判或无效清除；
    \item \textbf{终止判据触发一致性}：测试 1 与测试 2 触发定理 5 第二判据（确认全部空置频道后退出），测试 3 在清除第 16 个目标后直接触发第一判据即刻退出，与理论推导完全吻合；
    \item \textbf{时间合规与实时高效}：全任务虚拟时间最长为 $5884.9\text{ s}$（约 98 分钟），完全处于 100 分钟的时间限制之内；单步决策计算耗时低于 $0.1\text{ ms}$，计算效率极高，完全满足机器狗实时在线决策的工程要求。
\end{enumerate}
