...
本文研究四足机器狗自主搜索、定位并清除未知无线电干扰源的策略。机器狗运动速度固定，测向存在角度误差，且仅在近距离内才能实施有效清除。针对这些物理特点，本文结合几何分析与在线调度，依次解决多站交会定位、第二测点选址优化、多全向源协同巡航清除，以及全向与定向混合干扰源的包围搜索与清除问题。

针对问题一，研究测向误差下的交会定位多边形直径计算与圆盘覆盖判定。针对 $\pm 1^\circ$ 的测向误差，将各测站示向度表示为半平面不等式，利用半平面交算法求出定位凸多边形的所有极点；利用区域直径必在极点对处取得的性质，采用旋转卡壳算法求得最远对踵点间距。基准算例解得定位多边形几何直径为 $\mathbf{48.523\text{ m}}$，此时以该直径为直径的闭圆盘能够完全覆盖定位区域。但通过构造正三角形反例证明，一般情况下该圆盘并不能完全覆盖定位区域（1000 组随机测试的完全覆盖率仅为 $\mathbf{34.7\%}$）。经全顶点穷举与 8443 次单调性测试验证，算法结果完全一致。

针对问题二，以第二检测点坐标为决策变量，建立以最小化后验最差几何直径为目标的选点模型。在保证测点距初测可行域最远距离不超过 $995\text{ m}$（预留 $5\text{ m}$ 安全裕量）的安全前提下，通过离散网格搜索与三阶段优化，求得使后验最劣直径最小的第二检测点坐标为 $\mathbf{(839.64, 538.75)}$，机动距离为 $\mathbf{997.62\text{ m}}$。二次交会后，最差后验直径由单测站的 $1500.23\text{ m}$ 显著缩小至 $\mathbf{111.90\text{ m}}$，不确定区域压缩达 $\mathbf{92.5\%}$，有效拉大了交会视角并抑制了径向发散。

针对问题三，面向多个全向干扰源的自主搜索与清除任务，建立基准拓扑覆盖与在线调度模型。设计由中心站与外环 6 站（外环分布半径 $\mathbf{1140\text{ m}}$）组成的七点对称基准测站，证明在 $1000\text{ m}$ 保守感知半径下全域最大测距仅 $\mathbf{992.69\text{ m}}$，实现 $1800\text{ m}$ 目标圆域无盲区覆盖；证明了角楔交会凸外包始终包含目标真实位置。采用分层清除判定：定位中最小外接圆半径不超过 $\mathbf{19.5\text{ m}}$ 提前清除（预留 $0.5\text{ m}$ 数值稳健裕量）；主动测向预算用尽后半径不超过 $\mathbf{20\text{ m}}$ 直接清除；未收敛时由边长 $\mathbf{26\text{ m}}$ 的正方形网格遍历兜底（单元中心距角点最远约 $\mathbf{18.385\text{ m}}$）。在目标静止、误差有界、基准网完整执行与通信确定响应条件下，证明算法必在有限步内终止。官方模拟器三次正式测试中，清除率均为 $\mathbf{100\%}$，虚拟任务总时间平均 $\mathbf{3162.4\text{ s}}$（单源加权耗时 $\mathbf{243.3\text{ s}}$），程序实际运行时间平均仅 $\mathbf{4.215\text{ s}}$。

针对问题四，研究全向与半平面定向混合干扰源的搜索与清除问题。半平面定向源仅在特定 $180^\circ$ 内发射信号，背向存在盲区。利用超平面分离性质，证明了目标落入测站闭凸包内是保证任意未知朝向的定向源均能被至少一个测站探测到的充要条件。据此设计了由中心站与内外双环共 25 个基准测站构成的三角剖分网（最大边长 $\mathbf{983.60\text{ m}}$，内切圆半径 $\mathbf{1806.28\text{ m}}$），确保全域无漏检。在基准覆盖与网格兜底的主流程外，引入对称镜像与自适应尾部探测支路；100 组配对实验表明，该支路使平均耗时缩减 $\mathbf{109.8\text{ s}}$，无效网格清除尝试减少 $\mathbf{40.2\%}$。官方模拟器三次正式测试中，混合源清除率均达 $\mathbf{100\%}$，虚拟任务总时间平均 $\mathbf{6759.9\text{ s}}$（单源加权耗时 $\mathbf{533.7\text{ s}}$），程序实际运行时间平均仅 $\mathbf{5.897\text{ s}}$。

总体而言，本文模型建立在严密的几何覆盖与有界误差推导基础之上，算法紧密契合机器狗运动与传感器物理约束。在官方模拟器测试中，全向源与混合源清除率均保持 100\%，算法实际运行时间仅数秒（远低于 1200 秒上限），具备扎实的理论保证与高效的在线求解性能。
