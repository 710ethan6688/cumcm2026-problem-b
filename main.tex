...
\]
求解过程分为三个阶段。首先，根据测向误差确界将实测示向度转化为半平面约束，并通过求交得到定位多边形的有序极点序列。随后，利用凸分析极值原理证明区域直径必在极点处取得，通过旋转卡壳算法求取最远顶点对和区域几何直径。最后，以该点对连线为直径构造闭圆盘，并结合理论证明、几何反例和向量点积判据判断其覆盖性。
...
在确立凸外包多边形 $K_{c, k}^+$ 恒包真值后，机器狗需逼近目标并发起激光清除。机械行进存在 $\delta_p = 0.5\text{ m}$ 的综合到位偏差，而激光清除半径为 $20\text{ m}$，因此必须建立带有误差裕量的几何判据，以保证清除指令一旦发出便命中目标。另一方面，若多次测向因交会角退化而未能收敛，系统还需通过有限网格机制完成兜底清除。定义多边形 $K_{c, k}^+$ 的最小外接圆（MEB）半径为 $r(K_{c, k}^+)$，圆心为 $\mathbf{c}(K_{c, k}^+)$。
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.2em]
    \item 各类极限工况下的目标清除率均稳定达到 100\%。在 15\% 通信丢包或超时条件下，重试机制成功处理了全部 394 次未决请求。强制采用网格兜底后，任务耗时虽上升至 $74833.4\text{ s}$，但全部目标仍被清除。这一结果印证了判据 1 兜底机制与命题 4 有限步终止的理论正确性。
