...
\end{equation}
作为连续二维闭集，$\mathcal{R}$ 内包含无穷多个点，直接求解两点间最大间距属于连续极值优化问题。为实现确定性快速求解，关键在于将连续搜索降维至有限极点集合的离散比较。为此，本文给出并证明如下几何极值性质：

\noindent\textbf{性质 1（凸多边形几何直径极值性）}\quad 紧致凸多边形 $\mathcal{R}$ 的几何直径 $D$ 必在该多边形的某对极点（外轮廓顶点）之间取得，即：
\begin{equation}
...
性质 1 将连续搜索降维至 $\binom{h}{2}$ 对离散顶点比较，但全顶点穷举计算复杂度仍为 $O(h^2)$。根据对踵几何特征，两点最大间距必在相互平行的外切支撑线接触点（对踵点对）处取得。为此引入旋转卡壳算法，通过一对平行卡尺绕多边形旋转步进，仅需在 $O(h)$ 线性时间内遍历全部对踵候选，即可精确提取区域几何直径 $D$ 及最远顶点对 $(\mathbf{a}^*, \mathbf{b}^*)$。
...
在未知目标分布、有限感知半径、测量误差以及缺乏外部终止提示的严苛条件下，单纯依赖启发式搜索无法杜绝目标遗漏风险。为此，针对全域完全清除任务建立如下几何性质推导与判定准则体系：
第一阶段证明七点蜂窝几何全覆盖性，确保全场目标均可被可靠探测；
第二阶段证明多次测向交会区域的真值包络性，确保目标物理位置不被错误剔除；
第三阶段建立最小外接圆精准清除与有限网格兜底准则，保证激光清除成功率与收敛有限性；
第四阶段给出系统自主安全退出的任务终止准则。

\subsubsection{七点蜂窝几何全覆盖性}
...
\noindent\textbf{性质 2（七点蜂窝几何全覆盖性）}\quad 以保守感知下限 $R_{\min} = 1000\text{ m}$ 为半径的 7 个闭圆盘之并集，严格包含半径 $1800\text{ m}$ 的目标总区域 $\mathcal{D}_0$，即 $\mathcal{D}_0 \subset \bigcup_{m=0}^6 \mathcal{B}(\mathbf{q}_m, R_{\min})$。
...
\end{equation}
因全域最大距离严格小于保守感知半径，全场不存在探测盲区，目标区域全部点均被完全覆盖，性质得证。
\end{cumcmproof}
...
性质 2 确保了全域目标的几何可发现性。然而，一旦某一频道首次捕获测向信号，由于机载测向仪存在确界误差 $[-1^\circ, 1^\circ]$，多次观测交汇形成的可行域必须在数学上严格保持对真实物理位置的可靠包络，避免因几何求交裁剪而将真实目标错误剔除。为此，首先确立凸外包多边形的真值包含性质。

\noindent\textbf{性质 3（可行域真值包络性）}\quad 在任意步 $k \ge 0$，算法维护的凸外包多边形恒满足 $\mathbf{s}_c \in X_{c, k} \subseteq K_{c, k}^+$。

\begin{cumcmproof}
初始外切多边形满足 $\mathbf{s}_c \in \mathcal{P}_0$；假设第 $k$ 步包含性成立，步 $k$ 接收新方位角 $\hat{\theta}$ 时，因测向误差有界 $|\hat{\theta} - \theta^*| \le 1^\circ$，真值必落在角楔内，故更新集 $K_{c, k+1}^+ = K_{c, k}^+ \cap \mathcal{W}$ 恒包含 $\mathbf{s}_c$；当遭遇负观测或清除失败时，在相容集中剔除对应圆盘而在外包层维持凸性，包含性天然保持。由数学归纳法可知性质得证。
\end{cumcmproof}
...
\noindent\textbf{准则 1（最小外接圆精准清除与有限网格兜底准则）}\quad 设定位综合误差确界为 $\delta_p = 0.5\text{ m}$，则：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
...
\noindent\textbf{准则 2（全局任务自主终止准则）}\quad 若系统无未决通信事务，且满足下列双判据之一：
\begin{equation}
...
\noindent\textbf{性质 4（系统有限步收敛性）}\quad 在物理规则与确定性有界误差假设下，由算法 1 控制的系统必在有限离散动作步数内以 100\% 清除率完成任务并安全退出。

\begin{cumcmproof}
$\mathcal{Q}_7$ 基准搜索任务至多 $7 \times 20 = 140$ 步且不重复；存活目标主动定位测向受预算 $N_{\mathrm{loc\_max}} = 3$ 约束；未在预算内收敛的目标转入有限网格兜底，已尝试单元永久剔除，未访问网格每步严格递减 1，至多经有限步必成功清除；通信重试有限且各频道追踪状态单向转移（一旦确证空置或清除便不再变更），系统必然在有限步内严格满足准则 2 终止条件并退出。
\end{cumcmproof}
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.2em]
    \item 在各类极限工况中目标清除率均稳定达到 100\%；在 15\% 通信丢包或超时干扰下，重试机制成功处理了全部 394 次未决请求；在强制网格兜底工况下，虽因全面遍历使耗时上升至 $74833.4\text{ s}$，但依然实现全部目标清除，印证了准则 1 兜底机制与性质 4 有限步收敛性的理论正确性。
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.2em]
    ...
    \item \textbf{终止判据触发一致性}：测试 1 与测试 2 触发准则 2 第二判据（确认全部空置频道后退出），测试 3 在清除第 16 个目标后触发第一判据即刻退出，与理论推导吻合；
