...
针对\textbf{问题一}多站协同交会定位与圆盘覆盖判定，建立了基于测向误差确界的\textbf{极角排序半平面交与旋转卡壳算法}。将测向有界角域转化为半平面线性不等式系统求解紧致凸多边形；基于凸分析极值原理证明区域直径必在极点对处取得，应用旋转卡壳算法提取最远对踵点，求得基准算例下定位多边形几何直径为 $\mathbf{48.523\text{ m}}$。以最远极点对连线为直径构造闭圆盘，由钝角反例与向量点积判据证明圆盘并非恒能覆盖定位多边形；在 1000 组独立蒙特卡洛算例中，完全覆盖率为 $\mathbf{34.7\%}$。全顶点穷举比对表明算法求解一致率达 $\mathbf{100\%}$，8443 次单调性测试全部通过。
...
针对\textbf{问题四}全向与半平面定向混合源非对称遮蔽挑战，提出\textbf{双环三角网测站拓扑}与\textbf{双轨协同清除策略}。证明了双环三角网测站（最大间距 $\mathbf{983.60\text{ m}}$，内切圆半径 $\mathbf{1806.28\text{ m}}$）对任意未知朝向目标提供多视角可观测性保证；构建理论保证主链并结合自适应尾部前瞻探测轻量支路，抑制盲侧长尾机动。100 组配对对照实验表明启发支路使平均耗时缩减 $\mathbf{109.8\text{ s}}$（难例缩减 $\mathbf{646.5\text{ s}}$），无效尝试减少 $\mathbf{40.2\%}$。在官方模拟器三次正式测试中，混合源清除率达 $\mathbf{100\%}$，任务总耗时分别为 $\mathbf{5280.4\text{ s}}$、$\mathbf{5315.2\text{ s}}$ 与 $\mathbf{5308.6\text{ s}}$，平均为 $\mathbf{5301.4\text{ s}}$（单源耗时 $\mathbf{530.1\text{ s}}$）；航线轨迹见正文表 6 与支撑材料。
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
    \item \textbf{目标位置静止与基准坐标无漂移}：目标干扰源在整个作业过程中保持位置静止；机器狗自身定位与方位基准准确，坐标与航向角不存在系统漂移偏差。
    \item \textbf{信道信号来源单一且处于覆盖范围}：同一信道接收到的测向信号均源自该频点对应的单一目标，且参与交会计算的各测站均处于该目标的有效辐射范围内。
    ...
    \item \textbf{分段匀速直线机动与加减速忽略}：机器狗转向、起步与制动过程极为迅速，与长距离移动耗时相比可忽略不计，近似为以 $5\text{ m/s}$ 固定速度运动的质点分段匀速直线机动。
    \item \textbf{平坦地面视距传播与辐射源状态稳定}：电磁波在平坦二维地面内沿视距直线传播；目标干扰源在任务期间发射功率稳定且工作频率固定，不存在跳频或间歇关机。
\end{enumerate}
