...
综合状态演化、完备性检验与滚动调度，整体在线自主控制算法如算法 1 所示。考虑到实际运行中可能出现的无线通信延迟或指令丢包，系统向环境派发的每个动作请求附带唯一标识编号。若遇到未决状态（$\mathrm{pending}$），系统暂缓派发新任务，并重新发送当前动作指令进行状态同步，重试上限设为 $N_{\mathrm{retry}} = 3$ 次，以保证状态一致性并避免程序陷入等待死锁。
