...
本文研究四足机器狗对突发未知无线电干扰源的自主搜索定位与物理清除策略。针对干扰源参数未知、测向存在有界误差以及多目标在线搜索等现实约束，结合机器狗机动运动学与传感器角域特性，建立分层序贯决策与路径规划模型。采用计算几何与启发式在线调度算法，依次求解多站交会定位与圆盘覆盖判定、第二检测点优化选址、全向干扰源自主巡航清除及混合干扰源双轨协同清除问题。

针对问题一，建立测向误差确界约束下的交会定位与多边形几何直径解算模型。首先根据 $\pm 1^\circ$ 测向误差，将各站实测示向度转化为半平面不等式系统。采用极角排序半平面交算法，求解定位多边形的逆时针极点序列。基于凸分析极值原理证明区域直径必在极点对处取得，应用旋转卡壳算法提取最远对踵点。在基准算例下，求得定位多边形几何直径为 $\mathbf{48.523\text{ m}}$。以最远对踵点连线为直径构造闭圆盘，通过正三角形几何反例证明圆盘并非恒能覆盖定位区域。1000 组蒙特卡洛算例中完全覆盖率为 $\mathbf{34.7\%}$。经全顶点穷举验证，算法一致率达 $\mathbf{100\%}$，8443 次单调性测试全部通过。

针对问题二，建立以后验最劣几何直径最小化为目标的极小化极大 (Minimax) 优化模型。将第二检测点的二维空间坐标作为决策变量。针对初测示向角与感知极限限定的凸多边形包络，通过角平分线正交变换消除空间对称冗余。以最劣直径最小为目标、机动安全距离为约束进行离散网格搜索。求得最优第二检测点坐标为 $\mathbf{(839.64, 538.75)\text{ m}}$，机动距离为 $\mathbf{997.62\text{ m}}$。二次交会后，最劣后验直径从单站的 $1500.23\text{ m}$ 显著降至 $\mathbf{111.90\text{ m}}$，不确定空间压缩比达 $\mathbf{92.5\%}$。扩展实验进一步揭示了拉大交会视角、抑制径向拉伸的几何机理。

针对问题三，针对全域多全向干扰源协同清除任务，构建基于拓扑覆盖保证与滚动调度的自主巡航模型。利用平面覆盖理论，严格证明间距 $1732.05\text{ m}$ 的正六边形蜂窝测站对半径 $1800\text{ m}$ 目标圆域可实现无盲区覆盖，且交会多边形恒包含目标真值。确立以最小外接圆半径 $\mathbf{19.5\text{ m}}$ 为阈值的安全清除判据，设计网格遍历兜底与频道单向吸收退出准则，证明系统在有限步内必以 100\% 清除率终止。在官方模拟器三次正式测试中，全向源清除率均达 $\mathbf{100\%}$，任务总耗时分别为 $\mathbf{5102.1\text{ s}}$、$\mathbf{5145.6\text{ s}}$ 与 $\mathbf{5134.2\text{ s}}$，平均耗时 $\mathbf{5127.3\text{ s}}$，单步决策时间低于 $\mathbf{0.001\text{ s}}$。

针对问题四，针对全向与半平面定向混合干扰源的非对称遮蔽对抗，建立基于多视角包围拓扑与双轨协同决策的闭环清除模型。首先根据凸包分离性质，严格证明目标落入测站闭凸包是任意未知朝向全向可观测的充要条件。随后采用最大边长 $\mathbf{983.60\text{ m}}$、内切圆半径 $\mathbf{1806.28\text{ m}}$ 的 25 点双环三角网测站拓扑，实现对全域未知朝向目标的可靠发现。在理论保证主链基础上引入自适应尾部前瞻探测轻量支路，100 组配对对照实验表明启发支路使平均任务耗时缩减 $\mathbf{109.8\text{ s}}$（难例缩减 $\mathbf{646.5\text{ s}}$），无效尝试减少 $\mathbf{40.2\%}$。在官方模拟器三次正式测试中，混合源清除率达 $\mathbf{100\%}$，任务总耗时分别为 $\mathbf{5280.4\text{ s}}$、$\mathbf{5315.2\text{ s}}$ 与 $\mathbf{5308.6\text{ s}}$，平均耗时 $\mathbf{5301.4\text{ s}}$（单源耗时 $\mathbf{530.1\text{ s}}$）。
