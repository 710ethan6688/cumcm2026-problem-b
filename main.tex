...
\end{equation}
其中 $\eta_{\mathrm{clear}} = K/N = 1$。为此，本文兼顾理论完备性与在线调度时效性：一方面建立涵盖全域几何覆盖、真值区域包含、安全清除与自主退出的理论保证，在数学上杜绝目标遗漏；另一方面采用极小极大几何收缩准则与动态优先级评估，实现多频道目标在线交错搜索、追踪与就地清除，大幅压缩行进与测向时间。
...
在未知目标分布、有限感知半径、测量误差以及缺乏外部终止提示的严苛条件下，单纯依赖启发式搜索无法杜绝目标遗漏风险。为此，针对全域完全清除任务建立如下理论支撑体系：
第一阶段证明七点蜂窝几何覆盖定理，确保全场目标均可被探测发现；
第二阶段证明多次测向交会区域恒包真值，确保目标物理位置不被错误剔除；
第三阶段建立最小外接圆与有限网格判定准则，保证激光清除成功率与收敛有限性；
第四阶段给出系统自主安全退出的充分判定条件。
...
\noindent\textbf{定理 2（七点蜂窝全域覆盖定理）}\quad 以保守感知下限 $R_{\min} = 1000\text{ m}$ 为半径的 7 个闭圆盘之并集，严格包含半径 $1800\text{ m}$ 的目标总区域 $\mathcal{D}_0$，即 $\mathcal{D}_0 \subset \bigcup_{m=0}^6 \mathcal{B}(\mathbf{q}_m, R_{\min})$。
...
\noindent\textbf{定理 3（目标区域真值包含定理）}\quad 在任意步 $k \ge 0$，算法维护的凸外包多边形恒满足 $\mathbf{s}_c \in X_{c, k} \subseteq K_{c, k}^+$。
...
\noindent\textbf{定理 4（最小外接圆清除与网格兜底定理）}\quad 设定位综合误差确界为 $\delta_p = 0.5\text{ m}$，则：
\begin{enumerate}[leftmargin=2.5em, itemsep=0.15em]
...
在完成各个被探测目标的逐一清除后，系统面临的关键决策在于：在外部环境不提供剩余存活目标总数的情况下，机器狗何时可以确信全场已绝对无任何存活目标，从而自主决断终止任务？若盲目提前退出将造成漏检，若无止境游弋则会导致时间超时。为此，依据各频道追踪状态单向转移且不回退的特性，建立全局任务终止的充分判定条件。记已清除频道集为 $\mathcal{C}_k^{\mathrm{cl}}$，已证实空置频道集为 $\mathcal{C}_k^{\mathrm{abs}}$。

\noindent\textbf{定理 5（全局任务完备终止定理）}\quad 若系统无未决通信事务，且满足下列双判据之一：
\begin{equation}
...
\noindent\textbf{定理 6（系统有限步收敛定理）}\quad 在物理规则与确定性有界误差假设下，由算法 1 控制的系统必在有限离散动作步数内以 100\% 清除率完成任务并安全退出。

\begin{cumcmproof}
$\mathcal{Q}_7$ 基准搜索任务至多 $7 \times 20 = 140$ 步且不重复；存活目标主动定位测向受预算 $N_{\mathrm{loc\_max}} = 3$ 约束；未在预算内收敛的目标转入有限网格兜底，已尝试单元永久剔除，未访问网格每步严格递减 1，至多经有限步必成功清除；通信重试有限且各频道追踪状态单向转移（一旦确证空置或清除便不再变更），系统必然在有限步内严格触发定理 5 判据并退出。
\end{cumcmproof}
