...
本文研究机动无人平台（机器狗）对突发未知无线电干扰源的自主快速定位与物理清除策略。针对干扰源参数未知、测向存在有界物理误差及多目标在线交错搜索等约束，综合考虑平台运动学、传感器角域观测、信道正交性及离散动作时序，建立覆盖几何约束与信息状态演化的分层序贯决策体系。结合计算几何与启发式在线调度，依次求解多站交会定位与圆盘覆盖判定、第二检测点优化选址、全向干扰源自主巡航清除及混合干扰源双轨协同清除问题。

针对问题一，建立测向误差确界约束下的交会定位与多边形几何直径解算模型。基于误差半宽 $\pm 1^\circ$ 将实测示向度转化为闭半平面不等式系统，采用极角排序半平面交算法解出定位多边形的逆时针极点序列。基于凸分析极值原理证明区域直径必在极点对处取得，应用旋转卡壳算法提取最远对踵点，求得基准算例下定位多边形几何直径为 $\mathbf{48.523\text{ m}}$。以最远极点对连线为直径构造闭圆盘，由正三角形几何反例严格证明圆盘并非恒能覆盖定位区域；在 1000 组独立蒙特卡洛算例中，完全覆盖率为 $\mathbf{34.7\%}$。全顶点穷举比对表明算法求解一致率达 $\mathbf{100\%}$，8443 次单调性测试全部通过。

针对问题二，建立以后验最劣几何直径最小化为目标的极小化极大 (Minimax) 序贯优化模型。将第二检测点空间坐标作为决策变量。针对初测示向角与感知极限划定的外包络几何凸集，通过角平分线正交坐标变换消解对称冗余，以最劣直径最小化为目标、机动安全距离为约束进行离散网格寻优，求得最优第二检测点坐标为 $\mathbf{(839.64, 538.75)\text{ m}}$，机动距离为 $\mathbf{997.62\text{ m}}$。二次交会后，最劣后验直径由单站时的 $1500.23\text{ m}$ 锐减至 $\mathbf{111.90\text{ m}}$，不确定空间压缩比达 $\mathbf{92.5\%}$。扩展实验进一步揭示了拉大交会视角、抑制径向拉伸的几何机理。

针对问题三，针对全域多全向干扰源协同清除任务，构建基于拓扑覆盖保证与滚动时域调度的自主巡航模型。利用平面覆盖几何理论，严格证明了间距 $1732.05\text{ m}$ 的正六边形蜂窝测站对半径 $1800\text{ m}$ 目标圆域的无盲区覆盖，以及交会凸多边形恒包含目标真值；确立以最小外接圆半径 $\mathbf{19.5\text{ m}}$ 为阈值的精准清除判据，设计有限网格遍历兜底与全频道单向吸收退出准则，证明系统在有限步内必以 100\% 清除率终止。在官方模拟器三次正式测试中，全向源清除率均达 $\mathbf{100\%}$，任务总耗时分别为 $\mathbf{5102.1\text{ s}}$、$\mathbf{5145.6\text{ s}}$ 与 $\mathbf{5134.2\text{ s}}$，平均耗时为 $\mathbf{5127.3\text{ s}}$，单步决策时间低于 $\mathbf{0.001\text{ s}}$。

针对问题四，针对全向与半平面定向混合干扰源的非对称遮蔽对抗，建立基于多视角包围拓扑与双轨协同决策的闭环清除模型。首先根据凸包分离性质，严格证明了目标落入测站闭凸包是任意未知朝向全向可观测的充要条件。随后采用最大边长 $\mathbf{983.60\text{ m}}$、内切圆半径 $\mathbf{1806.28\text{ m}}$ 的 25 点双环三角网测站拓扑，实现对全域未知朝向目标的可靠发现。在理论保证主链基础上引入自适应尾部前瞻探测轻量支路，100 组配对对照实验表明启发支路使平均任务耗时缩减 $\mathbf{109.8\text{ s}}$（难例缩减 $\mathbf{646.5\text{ s}}$），无效尝试减少 $\mathbf{40.2\%}$。在官方模拟器三次正式测试中，混合源清除率达 $\mathbf{100\%}$，任务总耗时分别为 $\mathbf{5280.4\text{ s}}$、$\mathbf{5315.2\text{ s}}$ 与 $\mathbf{5308.6\text{ s}}$，平均耗时为 $\mathbf{5301.4\text{ s}}$（单源耗时 $\mathbf{530.1\text{ s}}$）。

方案均通过独立多算法交叉复核与极端工况压力测试，严格满足平台机动性能、动作时序与环境物理约束，兼具完备理论保障与毫秒级在线响应性能。

\vspace{-0.4em}
\cumcmkeywords{交会定位\quad 半平面交\quad 旋转卡壳\quad 蜂窝覆盖\quad 双环三角网\quad 极小化极大\quad 在线协同调度}
\end{cumcmabstract}
...
\]
求解过程分为三个阶段：首先，根据测向误差确界将实测示向度转化为半平面约束，并通过求交得到定位多边形的有序极点序列；随后，利用凸分析极值原理证明区域直径必在极点处取得，通过旋转卡壳算法求取最远顶点对和区域几何直径；最后，以该点对连线为直径构造闭圆盘，结合正三角形几何反例和向量点积判据判定其覆盖性。

\subsection{测向误差角域与半平面模型}

为实现上述交会定位与几何解算流程，首要环节是将物理测向数据转化为严密的几何约束。由于测向仪存在角度误差，单个检测点获得的观测并非单条理想射线，而是具有一定张角的扇形角域。因此，多站协同交会定位的本质是求取各测向角域在二维平面上的公共重叠区域。为此，本节利用双向半平面刻画单个检测点的前向观测角域，进而将联合交会定位区域 $\mathcal{R}$ 严格表示为有限闭半平面的交集 \cite{gholami2013}，为后续多边形顶点解算与几何直径计算提供标准的不等式输入。
...
在模型边界界定上，发射覆盖半径、5 米近场盲区和 1800 米目标圆边界共同决定测向信号能否被成功接收。本问已经给出获得有效示向度的各检测点，因此交会定位模型直接处理这些观测形成的半平面约束，不再引入先验圆弧进行二次裁剪，从而保持定位区域的凸多边形结构与算法求解的准确性。至此，由全体测向角域共同界定的定位区域被严格表征为包含 $2n$ 条线性不等式的闭凸集系统。
...
\end{equation}
本文采用基于双端队列的增量半平面求交算法。预处理阶段先提取全部 $2n$ 条有向边界的法向角并按极角排序，同时对同向平行边界仅保留约束最严格者。随后，利用双端队列维护当前可行区域的活动边界；每加入一个新半平面，若队头或队尾的相邻交点落入不可行侧，便执行相应的出队剪枝。完成全部半平面的遍历和闭合检验后，联立相邻边界方程得到逆时针排列的极点序列 $\mathcal{V}$。该算法将连续区域求解转化为有限边界极点计算，时间复杂度为 $O(n\log n)$，空间复杂度为 $O(n)$。
...
本文进一步从输入扰动、几何变换、约束演化和数值容差四个方面开展稳健性压力测试。对观测顺序进行 1000 次随机置换，并分别加入 1000 组同向冗余约束后，算法输出均保持不变。施加 $37^\circ$ 旋转和大距离平移时，区域直径同样保持不变。8443 次增量约束测试均满足直径单调非增性质；将临界容差调整为 $10^{-9}$ 和 $10^{-11}$ 时，求解状态与覆盖判定也保持稳定。双算法对照与稳健性检验结果汇总于表 \ref{tab:q1_verification_methods}。
...
记算法求得的最坏后验直径为 $\widetilde{D}_2 := \widetilde{J}_{64,7} = \widehat{J}_{64,7}(\widetilde{\mathbf{p}}_2)$，初始外包络几何直径为 $D_1^+ = \operatorname{diam}(\mathcal{P}_1^+)$。据此，定义离散评估下的不确定性压缩比为 $\widetilde{\eta} = (D_1^+ - \widetilde{D}_2)/D_1^+$。对基准算例（$\mathbf{p}_1 = (0, 0)\text{ m}, \hat\theta_1 = 0^\circ$）进行求解，单次计算耗时约 1.2 秒，指标如表 \ref{tab:q2_results} 所示。
...
为给出具体的较优候选区域，在基准算例下以网格步长 $h=25\text{ m}$ 对保守安全域 $\mathcal{U}$ 实施全域网格扫描。扫描范围取各安全圆盘公共区域的包围盒（$x\in[500, 1000]\text{ m}, y\in[-950, 950]\text{ m}$），共得到 702 个有效安全网格点。对这些网格点，采用与全域粗筛一致的 24 个代表源点和 3 个误差节点计算离散评估指标 $\widehat{J}_{24,3}$。全域网格最优值为 $111.893\text{ m}$，对应最优网格点 $(850, 525)\text{ m}$。各近优候选区域计算结果汇总于表 \ref{tab:q2_near_optimal}，其中区域面积按入选网格点数乘以单个网格单元面积（$h^2=625\text{ m}^2$）进行估算。
...
\end{equation}
其中 $\eta_{\mathrm{clear}} = K/N = 1$。为此，本文同时兼顾完全清除的理论保证和在线调度效率：理论层面依次建立全域几何覆盖 \cite{choset2001}、真值包含、安全清除和自主退出条件，在数学上杜绝目标遗漏；执行层面则采用极小极大几何收缩准则与动态优先级调度，使多个频道能够交错完成搜索、追踪与就地清除 \cite{song2012}，大幅压缩行进与测向时间。
...
在确立凸外包多边形 $K_{c, k}^+$ 恒包真值后，机器狗需逼近目标并发起激光清除。机械行进存在 $\delta_p = 0.5\text{ m}$ 的综合到位偏差，而激光清除半径严格为 $20\text{ m}$，因此需要建立带有误差裕量的几何判据，以保证清除指令必定命中目标。另一方面，若多次测向因交会角退化而未能收敛，系统还应通过有限网格机制完成兜底清除。定义多边形 $K_{c, k}^+$ 的最小外接圆（MEB）半径为 $r(K_{c, k}^+)$，圆心为 $\mathbf{c}(K_{c, k}^+)$。
...
\begin{cumcmproof}
基准搜索至多包含 $7 \times 20 = 140$ 个互不重复的检测动作，每个存活目标的主动测向次数又受预算 $N_{\mathrm{loc\_max}} = 3$ 限制。对于未在该预算内收敛的目标，算法转入有限网格兜底；由于已尝试单元永久剔除，未访问单元数每步严格减少，清除过程必在有限步内结束。此外，通信重试次数有限，各频道状态也只发生从未探明到确证空置或已清除的单向转移。因此，系统最终必在有限步内满足准则 2 并退出。
\end{cumcmproof}
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.05em, topsep=0.2em]
    \item 各类极限工况下的目标清除率均达到 100\%。在 15\% 通信丢包或超时条件下，重试机制处理了全部 394 次未决请求，说明通信恢复过程能够维持动作状态的一致性。强制采用网格兜底后，任务耗时上升至 $74833.4\text{ s}$，但全部目标仍被清除；这一结果与判据 1 和命题 4 所给出的有限步兜底结论一致。
...
为此，本文采取兼顾发现保证与搜索效率的协同求解思路 \cite{guvensan2011}：首先构建由 25 个测站组成的双环网络，利用凸包包围性质保证任意朝向的目标至少能够被一个测站发现。在此基础上，进一步引入距离安全选点、对称镜像探测和自适应尾部探测，以减少不必要的机动和网格遍历，同时保持完全清除的保证条件不变。
...
引理 1 保证了只要测站获得有效示向角，交会多边形便能稳健收缩并锁定真实目标。然而，仅凭单次测向获得的角楔面积依然巨大，必须机动至第二测站进行双向交会。在定向天线半平面发射背景下，若第二测站与目标的距离超出覆盖半径 $R_c$，不仅无法接收信号，且系统无法区分未收到信号究竟是因为距离超距还是落入目标盲区。第二检测点必须满足严格的距离安全准则，使无信号反馈不再受到距离超限因素的混淆。对于候选测站 $\mathbf{q}$，该准则要求其到多边形 $P_c^k$ 内任意可能目标位置的距离均不超过安全阈值。由于欧氏距离关于目标位置 $\mathbf{x}$ 为凸函数，上述最大距离可在顶点集 $\operatorname{Vert}(P_c^k)$ 上取得：
\begin{equation}
...
经过多测站交会收缩后，目标真实位置已被限定在紧致凸多边形 $P_c^k$ 内部。此时系统需要驱动机器狗实施物理清除。机载激光的有效清除半径为 $20\text{ m}$。若定位多边形已充分收缩，机器狗可停靠其几何中心并直接清除目标；反之，若多边形仍然较大或主动测向预算已经耗尽，系统便转入无遗漏的网格遍历。为此，建立外接圆直接清除与正方形网格遍历相结合的双轨判定准则：
...
第 6.3 节建立的 25 点基准拓扑、距离安全交会与网格遍历构成了确保 100\% 清除的理论保证主链。然而，在盲侧失联或定位区域仍较大时，直接转入网格遍历可能产生较高时间成本。因此，本文在保证主链之外增加两条轻量自适应决策支路 \cite{bercea2016}：盲侧失联时尝试对称镜像二次探测，定位多边形仍较大时自适应评估尾部追加探测的预期收益。两项机制只用于减少无效机动和网格遍历，不改变完全清除的判定条件。
